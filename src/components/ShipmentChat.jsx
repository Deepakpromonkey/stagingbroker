...
function ShipmentChat({ shipmentUuid, driverName }) {
    ...
    return (
        <div className="flex h-[460px] flex-col overflow-hidden rounded-xl border border-[#E2E8F0]">
            ...
            {error && (
                <p className="border-t border-[#FEE2E2] bg-[#FEF2F2] px-4 py-2 text-[11px] font-semibold text-[#B91C1C]">
                    {error}
                </p>
            )}

            <form onSubmit={send} className="flex items-center gap-2 border-t border-[#E2E8F0] bg-[#F8FAFC] px-3 py-3">
                <input
                    value={draft}
                    onChange={(e) => setDraft(e.target.value)}
                    maxLength={2000}
                    placeholder="Type a message to the driver"
                    className="flex-1 rounded-xl border border-[#E2E8F0] bg-white px-3.5 py-2.5 text-sm text-[#0F172A] outline-none transition focus:border-[#2563EB] focus:ring-2 focus:ring-[#DBEAFE]"
                />
                <button
                    type="submit"
                    disabled={sending || draft.trim() === ""}
                    className="flex items-center gap-1.5 rounded-xl bg-[#001A48] px-4 py-2.5 text-xs font-bold text-white transition hover:bg-[#1E293B] disabled:opacity-50"
                >
